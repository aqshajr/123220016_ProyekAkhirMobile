            ...
            onPressed: () async {
              if (formKey.currentState!.validate() && selectedTemple != null) {
                if (note == null) {
                  ...
                  // Update catatan
                  note.namaCandi = selectedTemple?.title ?? 'Tidak ada nama';
                  note.tanggalKunjungan = selectedDate;
                  note.kesanPesan = kesanPesanController.text;
                  await VisitNoteService.updateNote(note);
                }
                if (mounted) {
